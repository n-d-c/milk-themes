<div class="callingcard" class:hide={!display}>
	<div class="callcard">
		<h2 class="fn org">
			<span class="organization-name">{$wp?.site.organization}</span>
		</h2>
		<p class="category">{$wp?.site.tagline}</p>
		<h3 class="tel">{$wp?.site.phone}</h3>
	</div>
...
<script>
	...
	const makeCall = () => {
		window.location = `tel:+${$wp?.site?.telephone}`;
	};
	const addToContacts = () => {
		window.open($wp?.site?.vcf_file, 'contact');
	};
</script>
...
<style>
	...
	.org {
		color: var(--color-red-deep, #620b13);
	}
	.buttons {
		text-align: center;
		width: 100%;
