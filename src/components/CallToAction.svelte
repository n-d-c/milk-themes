<div class={`calltoaction ${add_class}`}>
...
<script>
	...
	const doEmail = () => {
		if (window?.location) {
			window.location = `mailto:${$wp?.site?.email_address}`;
		}
	};
	const doCalendar = () => {
...
<style>
	.calltoaction {
		...
		text-align: center;
		background: var(--background-red-vibrant, #800d16);
	}
	.calltoaction-inner {
		margin: 0 auto;
	...
	button.call-button:hover {
		border: 3px solid var(--color-yellow-vibrant, #f4ba38);
	}
	...
	hr {
		display: block;
		margin: 25px var(--padding-inner, 20px) 0;
		border: 0 none;
