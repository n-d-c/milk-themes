...
<script>
	...
	let credit_vite = $milk?.config?.credit?.vite ?? true;
	let credit_rollup = $milk?.config?.credit?.rollup ?? true;
	let credit_postcss = $milk?.config?.credit?.postcss ?? false;
	let credit_markdown = $milk?.config?.credit?.markdown ?? false;
	export {
		name,
		url,
		email,
