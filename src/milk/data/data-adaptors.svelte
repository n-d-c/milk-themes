<script>
	...
	const get = (
		url,
		request_swr = config.cache_swr,
		request_cache = config.cache,
		...
		const loadData = async () => {
			...
			if (config.cache_swr && request_swr) {
			...
			if (config.cache) {
				...
				if (cached_data && cached_data.data && cached_data.timestamp) {
					let cached_for = parseInt(
						(new Date().getTime() -
							new Date(cached_data.timestamp).getTime()) /
							1000
					);
			...
			if (!config.cache || !cache_good) {
				...
				if (config.cache) {
					idbSet(request_hash, { timestamp: new Date(), data: data });
				}
